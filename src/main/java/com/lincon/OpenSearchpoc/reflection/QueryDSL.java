...
public class QueryDSL<T> extends RangeSearchableQuery {
        ...
        for (Field field : fields) {
            ...
            if(fieldValue != null){
                if(field.isAnnotationPresent(Searchable.class)){
                    ...
                    QueryPromise queryPromise = new QueryPromise(searchable, fieldValue);
                }
            }
        }
    }
    ...
    private FieldValue getFieldValue(Field field) {
        ...
        try {
            if(field.get(filter) instanceof List<?>){
                field.get(filter);
            }
